...
public class Message {
	...
	/*
	 * public String packMessage(DataOutputStream outputStream){ try {
	 * outputStream.writeInt(this.messageType);
	 * 
	 * switch(this.messageType){ case 0: outputStream.writeUTF(this.ipAddress);
	 * outputStream.writeInt(this.port);
	 * 
	 * 
	 * 
	 * case 1: outputStream.writeUTF(this.ipAddress);
	 * outputStream.writeInt(this.port);
	 * 
	 * case 2: outputStream.writeInt(this.statusCode);
	 * outputStream.writeInt(this.identifier);
	 * outputStream.writeUTF(this.additionalInfo);
	 * 
	 * case 3: outputStream.writeInt(this.frontNodePort);
	 * outputStream.writeUTF(this.frontNodeIp);
	 * outputStream.writeInt(this.backNodePort);
	 * outputStream.writeUTF(this.backNodeIp);
	 * 
	 * 
	 * case 4: outputStream.writeInt(this.messagesToSend);
	 * 
	 * case 5: outputStream.writeInt(this.startNodeId);
	 * outputStream.writeInt(this.payload);
	 * 
	 * case 6: outputStream.writeInt(this.identifier);
	 * outputStream.writeUTF(this.ipAddress); outputStream.writeInt(this.port);
	 * 
	 * case 7: ; case 8: outputStream.writeUTF(this.ipAddress);
	 * outputStream.writeInt(this.port);
	 ...
	 * 
	 * 
	 * case 9: outputStream.writeInt(this.identifier);
	 ...
	 * }
	 * 
	 * outputStream.flush(); } catch (IOException e) { e.printStackTrace(); } return
	 * null; }
	 */
	 ...
	 * ArrayList<Object> messageContents = new ArrayList<Object>(); try {
	 * this.messageType = inputStream.readInt(); messageContents.add(messageType);
	 * 
	 * switch(this.messageType){ case 0: this.ipAddress = inputStream.readUTF();
	 * this.port = inputStream.readInt();
	 * 
	 * messageContents.add(ipAddress); messageContents.add(port);
	 * 
	 * case 1: this.ipAddress = inputStream.readUTF(); this.port =
	 * inputStream.readInt();
	 * 
	 * messageContents.add(ipAddress); messageContents.add(port);
	 * 
	 * case 2: this.statusCode = inputStream.readInt(); this.identifier =
	 * inputStream.readInt(); this.additionalInfo = inputStream.readUTF();
	 * 
	 * messageContents.add(statusCode); messageContents.add(identifier);
	 * messageContents.add(additionalInfo);
	 * 
	 * case 3: this.frontNodePort = inputStream.readInt(); this.frontNodeIp =
	 * inputStream.readUTF(); this.backNodePort = inputStream.readInt();
	 * this.backNodeIp = inputStream.readUTF();
	 * 
	 * messageContents.add(frontNodePort); messageContents.add(frontNodeIp);
	 * messageContents.add(backNodePort); messageContents.add(backNodeIp);
	 * 
	 * case 4: this.messagesToSend = inputStream.readInt();
	 * 
	 * messageContents.add(messagesToSend);
	 * 
	 * case 5: this.startNodeId = inputStream.readInt(); this.payload =
	 * inputStream.readInt();
	 * 
	 * messageContents.add(startNodeId); messageContents.add(payload);
	 * 
	 * case 6: this.identifier = inputStream.readInt(); this.ipAddress =
	 * inputStream.readUTF(); this.port = inputStream.readInt();
	 * 
	 * messageContents.add(identifier); messageContents.add(ipAddress);
	 * messageContents.add(port);
	 * 
	 * case 7: ; case 8: this.ipAddress = inputStream.readUTF(); this.port =
	 * inputStream.readInt(); this.numMessagesSent = inputStream.readInt();
	 * this.numMessagesReceived = inputStream.readInt(); this.sumOfSentMessages =
	 * inputStream.readInt(); this.sumOfReceivedMessages = inputStream.readInt();
	 * 
	 * messageContents.add(ipAddress); messageContents.add(port);
	 ...
	 * 
	 * case 9: this.identifier = inputStream.readInt(); this.frontNodePort =
	 * inputStream.readInt(); this.frontNodeIp = inputStream.readUTF();
	 * this.backNodePort = inputStream.readInt(); this.backNodeIp =
	 * inputStream.readUTF();
	 * 
	 * messageContents.add(identifier); messageContents.add(frontNodePort);
	 * messageContents.add(frontNodeIp); messageContents.add(backNodePort);
	 * messageContents.add(backNodeIp);
	 * 
	 * 
	 * } } catch (IOException e) { e.printStackTrace(); } return messageContents; }
	 */
